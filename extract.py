...
CREATE TABLE "projection" (
    ...
    with open('./data/traffic_data.json', encoding='utf-8') as file:
        for index, line in enumerate(file):
            ...
            for entry in data_set["t"]:
                cursor.execute('INSERT INTO datarecord(snapshot_id, x, y, n, l, i, rt, rd, d, c) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                    (snapshot_id, entry["x"], entry["y"], entry["n"].strip(), entry["l"].strip(), entry["i"].strip(), entry["rt"] if "rt" in entry else 0, entry["rd"], entry["d"], entry["c"]))
                datarecord_id = cursor.lastrowid
                for p in entry["p"]:
                    cursor.execute('INSERT INTO projection(datarecord_id, x, y, t, d) VALUES (?, ?, ?, ?, ?)',
                        (datarecord_id, p["x"], p["y"], p["t"], p["d"] if "d" in p else None))
            if index > 3000: break #1GB Limit
